...
fun Context.observeNetworkAvailable(): Observable<Boolean> =
   Observable.create {
    try {
      val connectivityManager = this.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
      val networkInfo = connectivityManager.activeNetworkInfo
      it.onNext(networkInfo != null && networkInfo.isAvailable)
...
fun Context.observeNetworkState(): Observable<NetworkInfo.State> =
   Observable.create<NetworkInfo.State>({
    val receiver = object : BroadcastReceiver() {
      override fun onReceive(context: Context, intent: Intent) {
        ...
        if (info != null) {
          it.onNext(info.state)
        } else {
          it.onNext(NetworkInfo.State.UNKNOWN)
        }
      }
    }
    ...
    it.setDisposable(object : MainThreadDisposable() {
      override fun onDispose() {
        this@observeNetworkState.unregisterReceiver(receiver)
      }
    })

    this.registerReceiver(receiver, IntentFilter("android.net.conn.CONNECTIVITY_CHANGE"))
